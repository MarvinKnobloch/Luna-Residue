...
public class Newbowattack : MonoBehaviour
{
    ...
    private void hookshotstart() => movementscript.state = Movescript.State.Bowhookshot;
    private void pullarrow() => prefabarrow.SetActive(true);
    private void arrowfalse() => prefabarrow.SetActive(false);
    ...
    {
        RaycastHit hit;
        if (Physics.Raycast(Kamerarichtung.position, Kamerarichtung.forward, out hit, Mathf.Infinity, Arrowraycastlayer, QueryTriggerInteraction.Ignore))
        {
        ...
        else
        {
            ...
            GameObject Arrow = Instantiate(arrowtyp, Arrowlaunchposi.position, Quaternion.LookRotation(arrowrotation, Vector3.up));
            SingleAirarrow arrowcontroller = Arrow.GetComponent<SingleAirarrow>();
            arrowcontroller.arrowziel = Kamerarichtung.position + Kamerarichtung.forward;
