...
public class Statueback : MonoBehaviour, Interactioninterface
{
    ...
    public bool Interact(Closestinteraction interactor)
    {
        {
            RaycastHit hit;
            Ray ray = new Ray(transform.parent.position, transform.parent.up * -0.5f - transform.parent.forward);
    ...
    public void movestatues()
    {
        ...
        {
            RaycastHit hit;
            Ray ray = new Ray(obj.transform.position, obj.transform.up * -0.5f - obj.transform.forward);
            if (Physics.Raycast(ray, out hit, 3, layer, QueryTriggerInteraction.Ignore) == false)
            {
                obj.transform.position = obj.transform.position + transform.forward * -3;
            }
        }
    }
}
