...
namespace Cinemachine
{
    ...
    public class Camcollider : CinemachineExtension
    {
        public float normalcamdistance;
        public CinemachineVirtualCamera vcam;
        private float resetcamtime;
        private float overtime;
        private float currentcamdistance;
        public LayerMask resetcamlayer;
        ...
        private Vector3 PreserveLineOfSight(ref CameraState state, ref VcamExtraState extra)   //muss drinnen bleiben
        {
            Vector3 displacement = Vector3.zero;
                ...
                && m_CollideAgainst != m_TransparentLayers)
            {
                ...
                displacement = PullCameraInFrontOfNearestObstacle(cameraPos, lookAtPos, m_CollideAgainst & ~m_TransparentLayers, ref hitInfo);
                ...
                {
                    if (framingTransposer !=null && framingTransposer.m_CameraDistance < normalcamdistance)
                    { 
                        resetcamtime += Time.deltaTime;
                        if (resetcamtime > 0.3)
                        {
                            Collider[] cols = Physics.OverlapSphere(transform.position, 0.5f, resetcamlayer);
                            if (cols.Length == 0)
                            {
                                overtime += Time.deltaTime * 4;
                                framingTransposer.m_CameraDistance = overtime;
                            }
                            else
                            {
                                resetcamtime = 0f;
                ...
                else
                {
                    ...
                    if (rayLength > Epsilon)
                    {
                            ...
                            ray, out hitInfo, rayLength, layerMask, m_IgnoreTag))
                        {
                            ...
                            float camdistance = (float)(Math.Round(adjustment, 2) - 0.3);
                            currentcamdistance = framingTransposer.m_CameraDistance;
                            if(camdistance > currentcamdistance + 0.7f)
                            {
                                currentcamdistance += Time.deltaTime * 4;
                                framingTransposer.m_CameraDistance = currentcamdistance;
                                overtime = framingTransposer.m_CameraDistance;
                            }
                            if(camdistance < currentcamdistance - 0.7f)
                            {
                                currentcamdistance -= Time.deltaTime * 4;
