...
namespace DecalSystem {
    ...
    public static class TerrainUtils {
        ...
        private static Matrix4x4 GetLocalToWorldMatrix(Terrain terrain) {
            var width = terrain.terrainData.heightmapResolution - 1;     //terrain.terrainData.heightmapWidth - 1;
            var height = terrain.terrainData.heightmapResolution - 1; //terrain.terrainData.heightmapHeight - 1;
        ...
        private static void GetMinMax(Bounds bounds, TerrainData terrain, out Vector3Int min, out Vector3Int max) {
            ...
            max.x = Mathf.Min( max.x, terrain.heightmapResolution - 1 );                     //terrain.heightmapWidth
            max.z = Mathf.Min( max.z, terrain.heightmapResolution - 1 );                    //terrain.heightmapHeight
        }
